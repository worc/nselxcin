...
async function getAllPhrases() {
  const result = await query(`SELECT phrase_id, salish, english, octet_length(salish_audio) AS size FROM phrases`)
  return result.rows
}
...
router.route('/phrase/:id/audio')
  .all(mpegParser)
  .get(async (req, res) => {
    const text = `SELECT salish_audio, octet_length(salish_audio) AS size FROM phrases WHERE phrase_id = $1`
    const values = [ req.params.id ]
    const result = await query(text, values)
    const phrase_audio = result.rows[0]

    res.set('Content-Type', 'audio/mpeg')
    res.set('Content-Length', `${phrase_audio.size}`)

    res.send(phrase_audio.salish_audio)
  })
  .put(async (req, res) => {
